...
*/
  ...
  if ($attend_result = $ins_stmt_attend->fetch()) {
    ...
    echo "Success! The ID of the attendance information entered is " . htmlspecialchars($attendance_id, ENT_QUOTES, 'UTF-8') . ".";
  }
  else {
  ...
  if(is_graded_class($class_id) and ($present == TRUE)) {
    ...
    foreach ($grade_types as $grade_type) {
      ...
      if ($grade_result = $ins_stmt_grade->fetch()) {
        ...
        echo "Success! The ID of the grade instance information entered is " . htmlspecialchars($ginstance_id, ENT_QUOTES, 'UTF-8') . ".";
      }
      else {
        // Insert failure, return error
        echo "Sorry, that didn't work. Error message: ";
